...
MODO = 'DEMO' # Mude para 'REAL' para operar na conta

client = Client(API_KEY, API_SECRET)

def analisar_fluxo():
    try:
        # Puxa H1 para Macro e M15 para Gatilho
        ...
        fundo_m15 = df_m15['l'].astype(float).iloc[-15:].min() # Origem do Movimento
        
        # Lógica: Preço acima do topo anterior (BOS) e H1 em estrutura de alta
        if preço > topo_h1:
...
print("🚀 METATRON V7: ANALISANDO ORIGEM DO FLUXO...")
while True:
    status, sl = analisar_fluxo()
    print(f"[{time.strftime('%H:%M:%S')}] Status: {status} | Stop na Origem (M15): {sl}")
    if status == "COMPRA":
        print(f"--- SINAL DETECTADO ({MODO}) ---")
        # Aqui entra a execução de ordem se MODO == 'REAL'
        time.sleep(3600)
    time.sleep(60)
